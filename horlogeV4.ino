...
bool atop,abot,amiddle,across;
unsigned long tempsappuilong = 1500; ///(1seconde 5)

// gestion des etats
enum State_enum{NORMAL,CHANGEHOUR1,CHANGEHOUR2,CHANGEHOUR3,CHANGEHOUR4,CHANGEHOUR5,CHANGEHOUR6,DISPLAYALARM1,DISPLAYALARM2,ALARM1,ALARM2};
uint8_t state = NORMAL;
// variable pour compter les appui long
unsigned long amiddlepressed;
bool amiddlecounting=false;
...
//
void leddisplay(uint8_t Hour,uint8_t Minute,bool D1,bool D2,bool D0){
...
void state_machine_run(){
  ...
  switch(state){
    case NORMAL: // cas normal on affiche l'heure
      ...
      if (alarm1.Hour==hour() && alarm1.Minute==minute()){state=ALARM1; break;} /// afini 
      if (alarm2.Hour==hour() && alarm2.Minute==minute()){state=ALARM2; break;} // a finir
      break;
      ...
      leddisplay(alarm2.Hour,blinck*alarm2.Minute,false,blinck,blinck);
       ...
       //onfait du bruit
       music();
        if(atop or abot or amiddle or across ){state=NORMAL; break;} // quand on appuis sur un bouton on retourne a normalnormal
        break;
       ...
       //onfait du bruit 
       music();
       if(atop or abot or amiddle or across ){state=NORMAL; break;} // quand on appuis sur un bouton on retourne a normal
       break;
...
void  setalarm(){
  if(EEPROM.read(0)!=1){ // c'est la premiere fois que l'horloge est configurées
    EEPROM.write(0, 1);
    EEPROM.write(1, 12);
    EEPROM.write(2, 12);
    EEPROM.write(3, 12);
